...
def infer(response_vectors: np.ndarray) -> str:
    ...
    * predicted_class: str
    ...
    """
    if not isinstance(response_vectors, np.ndarray):
        raise ValueError("response_vectors must be a numpy array")
    if response_vectors.shape[2] != 3:
        raise ValueError("response_vectors must have 3 channels")
    if np.min(response_vectors) < 0 or np.max(response_vectors) > 1:
...
def find_reference_region(image: np.ndarray, threshold: int = -1, stride: int = 1):
    ...
    * box_sums: list[int]
    ...
    convolved_image = fftconvolve(image.astype(np.float32), kernel, mode="valid")

    # Find the location of the minimum sum in the convolved image down to the pixel before stride
    min_sum_idx = np.unravel_index(np.argmin(convolved_image), convolved_image.shape)
...
def predict(pattern_type: str, response_vectors: np.ndarray, model_path: str):
    ...
    * window_size: int
    ...
    if not isinstance(response_vectors, np.ndarray):
        raise ValueError("response_vectors must be a numpy array")
    if response_vectors.shape[2] != 29:
        raise ValueError("response_vectors must have 29 channels")
